...
function read() {
    if (!file.exists(constants_2.CONFIG_FILE_PATH)) {
    ...
    const configRaw = file.read(constants_2.CONFIG_FILE_PATH);
    let config;
    try {
        config = JSONC.parse(configRaw);
    }
    catch (err) {
        console.error(`Failed to parse "${chalk_1.default.green(constants_2.CONFIG_FILE_PATH)}":`, err);
        process.exit(1);
    }
