...
function exists(filePath) {
    let pathExists;
    try {
        pathExists = fs_1.default.existsSync(filePath);
    }
    catch (err) {
...
function getDirList(dirPath) {
    let fileList;
    try {
        fileList = fs_1.default.readdirSync(dirPath);
    }
    catch (err) {
...
function getFileStats(filePath) {
    let fileStats;
    try {
        fileStats = fs_1.default.lstatSync(filePath);
    }
    catch (err) {
...
function read(filePath) {
    let fileContents;
    try {
        fileContents = fs_1.default.readFileSync(filePath, "utf8");
    }
    catch (err) {
