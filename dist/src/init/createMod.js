...
function copyStaticFiles(projectPath) {
    ...
    const vsCodeFileList = file.getDirList(constants_1.TEMPLATES_VSCODE_DIR);
    vsCodeFileList.forEach((fileName) => {
        const templateFilePath = path_1.default.join(constants_1.TEMPLATES_VSCODE_DIR, fileName);
        const thisFilePath = path_1.default.join(projectPath, fileName);
        if (!file.exists(thisFilePath)) {
            file.copy(templateFilePath, thisFilePath);
        }
    });
