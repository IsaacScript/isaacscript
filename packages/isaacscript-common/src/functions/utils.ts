 ...
 * API.
 *
 * This function should always be used over the `REPENTOGON` constant, since the latter is not safe.
 *
 * Specifically, this function checks for the `Sprite.StopOverlay` method, which was added in
 * REPENTOGON:
 * https://repentogon.com/Sprite.html#stopoverlay
 */
export function isRepentogon(): boolean {
  ...
  const getAnimation = classTable.get("StopOverlay");
  return isFunction(getAnimation);
}
