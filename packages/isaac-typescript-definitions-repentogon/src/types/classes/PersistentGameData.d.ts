...
declare global {
  ...
  interface PersistentGameData extends IsaacAPIClass {
    ...
    readonly IncreaseEventCounter: (event: EventCounter, count: int) => void;

    /** Returns whether the boss has been killed. This is used for tracking numerous unlocks. */
    readonly IsBossKilled: (bossID: BossID) => boolean;

    /** Returns whether the provided `Challenge` has been completed. */
    readonly IsChallengeCompleted: (challenge: Challenge) => boolean;

    /** Returns whether the provided collectible is in the items page in the stats menu. */
    readonly IsItemInCollection: (collectible: CollectibleType) => boolean;
    ...
    readonly Unlocked: (unlock: Achievement) => boolean;
  }
}
