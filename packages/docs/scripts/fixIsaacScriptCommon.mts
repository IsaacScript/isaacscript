...
async function moveModulesFiles() {
  ...
  for (const markdownFileName of markdownFileNames) {
    ...
    const separatorIndex = markdownFileName.indexOf("_");
    if (separatorIndex <= 0) {
      throw new Error(
    ...
    const directoryName = markdownFileName.slice(0, separatorIndex);
    const newFileName = markdownFileName.slice(separatorIndex + 1);
...
`.trimStart();
  ...
  const linesToRemove = DIR_NAMES_WITH_SECOND_BREADCRUMBS_LINE.has(
    directoryName,
  )
    ? 4
    : 2;

  fileContents = lines.slice(linesToRemove).join("\n");
...
function getTitle(filePath: string, directoryName: string) {
  ...
  if (DIR_NAMES_WITH_SECOND_BREADCRUMBS_LINE.has(directoryName)) {
    const properName = pageName.split(".").at(-1);
    if (properName === undefined || properName === "") {
      throw new Error(
...
async function renameDuplicatedPages() {
  for (const directoryName of DIR_NAMES_WITH_SECOND_BREADCRUMBS_LINE) {
    ...
    for (const fileName of fileNames) {
      ...
      const match = /\.(?<properName>\w+\.md)$/v.exec(fileName);
...
async function fixLinks() {
  const markdownFilePaths = fs.globSync("**/*.md", { cwd: PACKAGE_DOCS_DIR });
  ...
  for (const filePath of markdownFilePaths) {
    ...
    const linkFileNameMatches = newFileContents.matchAll(
      /\b(?<linkFileName>\w+\.md)/gv,
    );
    for (const match of linkFileNameMatches) {
      const linkFileName = match.groups?.["linkFileName"];
      if (linkFileName !== undefined) {
    ...
    for (const brokenLinkDirName of BROKEN_LINK_DIR_NAMES) {
      ...
      const brokenLink = new RegExp(
        String.raw`\(\.*/*${brokenLinkDirName}_`,
        "gm",
      );
...
function getNumDirectoriesAwayFromRoot(filePath: string): number {
