...
`.trimStart();
  ...
  // Remove the generated title, breadcrumbs link, and the blank line after each one.
  fileContents = lines.slice(4).join("\n");
...
async function renameEnumFiles() {
  ...
  for (const markdownFileName of markdownFileNames) {
    ...
    const match = /\.(?<simplifiedFileName>\w+\.md)$/v.exec(markdownFileName);
