...
const FILE_NAME_TO_TITLE: ReadonlyMap<string, string> = new Map([
  ...
  // Features
  ["ExtraConsoleCommandsInit", "Extra Console Commands (Init)"],
  ["ExtraConsoleCommandsList", "Extra Console Commands (List)"],
...
function fixTitles() {
  for (const titleFixDirectory of TITLE_FIX_DIRECTORIES) {
    const directoryPath = path.join(PACKAGE_DOCS_DIR, titleFixDirectory);
    const markdownFileNames = getMarkdownFileNames(directoryPath);

    for (const markdownFileName of markdownFileNames) {
      const markdownPath = path.join(directoryPath, markdownFileName);
      fixTitle(markdownPath);
    }
  }
}

function fixTitle(filePath: string) {
  const title = getTitle(filePath);
  const editURL = getEditURL(filePath);
  const header = `
---
title: ${title}
...
function getEditURL(filePath: string) {
  return "https://github.com/IsaacScript/isaacscript/edit/main/packages/isaacscript-common/src/core/cachedClasses.ts";
}
