...
declare global {
  ...
  const enum StageAPILayoutButtonVariant {
    ROOM_CLEAR = 0,
    REWARD = 1,
    GREED = 2,
    KILL = 9,
  ...
  const enum StageAPILayoutCornyPoopSubtype {
    NORMAL = 0,
    NON_REPLACEABLE = 1,
  }

  const enum StageAPILayoutGridType {
    ROCK = 1000,
    ROCK_ALT = 1002,
    ROCK_BOMB = 1001,
    ROCK_SPIKE = 1010,
  ...
  const enum StageAPILayoutPitfallVariant {
    NORMAL = 0,
    SUCTION = 1,
    TELEPORT = 2,
  }

  const enum StageAPILayoutPoopSubtype {
    NORMAL = 0,
    NON_REPLACEABLE = 1,
  }

  const enum StageAPILayoutRockSubtype {
    NORMAL = 0,
    NON_REPLACEABLE = 1,
  }

  const enum StageAPILayoutSpikesOnOffVariant {
    NORMAL = 0,
    DOWN_1_FIFTH = 1,
    DOWN_2_FIFTHS = 2,
    DOWN_3_FIFTHS = 3,
  ...
  const enum StageAPICallback {
     ...
     * entity list. Any return value breaks out of future callbacks. Every entity in the final
     * entity list is spawned. Note that this entity list contains EntityInfo tables rather than
     * EntityData, which contain persistent room-specific data. Both detailed below.
     */
